...
class HuffmanCoder {
  ...
  destringify(data) {
    //creating tree again from string tree structure
    let node = [];
    if (data[this.ind] === "'") {
      this.ind++;
  ...
  getMappings(node, path) {
    ...
    if (typeof node[1] === "string") {
      //leaf node detection(will be be a char(a|b|c|d etc) js identifies char as type string)
  ...
  encode(data) {
    //function to encode //will return values to script.js
    ...
    let padding = "";
    for (let i = 0; i < rem; i++) padding = padding + "0"; //adding additional 0s
